...
public class SyntheaTRANSFORM_REMOVE_FINDING {

private CompleteCollection CC;
	
	public SyntheaTRANSFORM_REMOVE_FINDING(CompleteCollection cC) {
		CC=cC;
	}
...
//		List<CompleteDocuments> A_Borrar=new LinkedList<CompleteDocuments>();
		...
		System.out.println("TA0="+CCSalida.getEstructuras().size());
		...
		for (CompleteDocuments completeDocuments : CC.getEstructuras()) {
			for (CompleteElement completeelemento2Doc : completeDocuments.getDescription()) {
				if (completeelemento2Doc instanceof CompleteTextElement
						...
						&&
						Pattern.compile("(finding)").matcher(((CompleteTextElement) completeelemento2Doc).getValue().toLowerCase()).find()
						)
		...
		System.out.println("TA1="+CCSalida.getEstructuras().size());
	...
	public static void main(String[] args) {
		System.out.println(Pattern.compile("(finding)").matcher("Fever (finding)").find());
		System.out.println(Pattern.compile("(finding)").matcher("Suspected COVID-19").find());
	}
}
